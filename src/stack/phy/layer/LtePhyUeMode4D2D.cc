...
void LtePhyUeMode4D2D::handleAirFrame(cMessage* msg)
{
    ...
    if (r.size() > 1)
    {
        ...
        {
            EV << "LtePhyUeMode4D2D: Receiving Packet from antenna " << (*it) << "\n";
...
void LtePhyUeMode4D2D::storeAirFrame(LteAirFrame* newFrame)
{
    ...
    if (strcmp(par("d2dMulticastCaptureEffectFactor"),"RSRP") == 0)
    {
        ...
        EV << NOW << " LtePhyUeMode4D2D::storeAirFrame - Average RSRP from node " << newInfo->getSourceId() << ": " << rsrpMean ;
    }
    else  // distance
    {
        Coord newSenderCoord = newInfo->getCoord();
        distance = myCoord.distance(newSenderCoord);
        EV << NOW << " LtePhyUeMode4D2D::storeAirFrame - Distance from node " << newInfo->getSourceId() << ": " << distance ;
    }
...
LteAirFrame* LtePhyUeMode4D2D::extractAirFrame()
{
    // implements the capture effect
...
void LtePhyUeMode4D2D::decodeAirFrame(LteAirFrame* frame, UserControlInfo* lteInfo)
{
